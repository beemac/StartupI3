...
var configDefaults = {
    ...
    faces: {
        ...
        maxResults: 4,
        headerLabel:        '',     //Label for a header which would appear above the results
        moreResultsLabel:   'See more results on BluePages (${count})',
        moreResultsLabel2:  'See more results on BluePages (over ${count})',
        moreResultsUrl:     'https://w3.ibm.com/bluepages/?s=${query}',
        ...
        onclick: function(person) {
            location.href=protocol + '//w3.ibm.com/bluepages/profile.html?uid='+person.uid;
...
var me = window.FacesTypeAhead = {
    ...
    config: {},
    initclass : function (multiclass, onclick_function) {
        var config = {
                key: "score;jdpolo@us.ibm.com",
                minQueryLength: 1,
                searchDelay: 0,
                faces: {
        ...
        var x = document.getElementsByClassName(multiclass);
        var i;
        for (i = 0; i < x.length; i++) {
        	if (!x[i].hasAttribute("autocomplete")) {
        		console.log("setting faces");
        		FacesTypeAhead.facesinit(x[i], config);
        	}
        }
	},
	init : function (dom_id, onclick_function) {
        var config = {
                key: "score;jdpolo@us.ibm.com",
                minQueryLength: 1,
                searchDelay: 0,
        ...
        FacesTypeAhead.facesinit(document.getElementById(dom_id), config);
	},
...
var TypeAhead = function (inputBox, settings) {
    ...
    var buildPersonFragment = function(user, size) {
        ...
        // As sunset of faces services, image enpoint is to unfied profile endpoint
        // img.src = [settings.faces.imageHost, ':', port, '/image/', user.uid, '.jpg?s=', imageSize].join('');
        
        img.src = [settings.faces.imageHost, '/image/', user.uid].join('');

        img.setAttribute('alt', user.name);
        img.setAttribute('onerror', "this.onerror=null; this.src='https://w3.ibm.com/bluepages/images/profile.jpg';")
